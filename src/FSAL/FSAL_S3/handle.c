...
/* handle.c
 * S3 object (file|dir) handle object
 */
...
static void s3_release(struct fsal_obj_handle *obj_hdl);

/* Atomic uint64_t that is used to generate inode numbers in the mem FS */
uint64_t s3_inode_number = 1;

/* helpers
 */

static inline int
s3_n_cmpf(const struct avltree_node *lhs,
		const struct avltree_node *rhs)
{
	struct s3_dirent *lk, *rk;

	lk = avltree_container_of(lhs, struct s3_dirent, avl_n);
	rk = avltree_container_of(rhs, struct s3_dirent, avl_n);

	return strcmp(lk->d_name, rk->d_name);
}

static inline int
s3_i_cmpf(const struct avltree_node *lhs,
		const struct avltree_node *rhs)
{
	struct s3_dirent *lk, *rk;

	lk = avltree_container_of(lhs, struct s3_dirent, avl_i);
	rk = avltree_container_of(rhs, struct s3_dirent, avl_i);
...
/**
 * @brief Construct the fs opaque part of a s3 nfsv4 handle
 *
 * Given the components of a s3 nfsv4 handle, the nfsv4 handle is
 ...
 *
 * @return The nfsv4 s3 file handle as a char *
 */
static void package_s3_handle(struct s3_fsal_obj_handle *myself)
{
	char buf[MAXPATHLEN];
	uint16_t len;
	uint64_t hashkey;
	 ...
	 * Ideally we can include entire s3 name for guaranteed
	 * uniqueness of s3 handles.
	 */
	...
	/* If there is more space in the opaque handle due to a short s3
	 * path ... zero it.
	 */
 ...
 */
static void s3_insert_obj(struct s3_fsal_obj_handle *parent,
			   struct s3_fsal_obj_handle *child,
			   const char *name)
{
...
s3_dirent_lookup(struct s3_fsal_obj_handle *dir, const char *name)
{
	...
	return avltree_container_of(node, struct s3_dirent, avl_n);
}
 ...
 */
static void s3_remove_dirent_locked(struct s3_fsal_obj_handle *parent,
				     struct s3_dirent *dirent,
				     bool release)
{
	struct s3_fsal_obj_handle *child;
	 ...
	 * with @r s3_insert_obj since that takes the locks seqentially */
	child = dirent->hdl;
	//PTHREAD_RWLOCK_wrlock(&child->obj_handle.obj_lock);
 ...
 */
static void s3_remove_dirent(struct s3_fsal_obj_handle *parent,
			      const char *name)
{
	struct s3_dirent *dirent;
...
//	PTHREAD_RWLOCK_wrlock(&parent->obj_handle.obj_lock);
	...
	dirent = s3_dirent_lookup(parent, name);
	if (dirent)
		s3_remove_dirent_locked(parent, dirent, false);
...
void s3_clean_export(struct s3_fsal_obj_handle *root)
{
	...
	while ((node = avltree_first(&root->mh_dir.avl_name))) {
		...
		if (child->obj_handle.type == DIRECTORY) {
			s3_clean_export(child);
		}

		/*PTHREAD_RWLOCK_wrlock(&root->obj_handle.obj_lock);*/
		s3_remove_dirent_locked(root, dirent, true);
...
void s3_clean_all_dirents(struct s3_fsal_obj_handle *parent)
{
	...
	while ((node = avltree_first(&parent->mh_dir.avl_name))) {
		dirent = avltree_container_of(node, struct s3_dirent, avl_n);
		s3_remove_dirent_locked(parent, dirent, true);
	}

	/*PTHREAD_RWLOCK_unlock(&parent->obj_handle.obj_lock);*/
}

static void s3_copy_attrs_mask(struct attrlist *attrs_in,
 ...
 * @return FSAL status
 */
static fsal_status_t s3_open_my_fd(struct fsal_fd *fd,
 ...
 * @return FSAL status
 */
static fsal_status_t s3_close_my_fd(struct fsal_fd *fd)
{
...
static fsal_status_t s3_open_func(struct fsal_obj_handle *obj_hdl,
				   fsal_openflags_t openflags,
				   struct fsal_fd *fd)
{
	return s3_open_my_fd(fd, openflags);
}
 ...
 * @return FSAL status
 */
static fsal_status_t s3_close_func(struct fsal_obj_handle *obj_hdl,
				    struct fsal_fd *fd)
{
	return s3_close_my_fd(fd);
}

#define s3_alloc_handle(p, n, t, e, a) \
	_s3_alloc_handle(p, n, t, e, a, __func__, __LINE__)
/**
 * @brief Allocate a S3 handle
 *
 * @param[in] parent	Parent directory handle
 * @param[in] name	Name of handle to allocate
 * @param[in] type	Type of handle to allocate
 * @param[in] mfe	S3 Export owning new handle
 * @param[in] attrs	Attributes of new handle
 * @return Handle on success, NULL on failure
 */
static struct s3_fsal_obj_handle *
_s3_alloc_handle(struct s3_fsal_obj_handle *parent,
		  const char *name,
		  ...
		  const char *func, int line)
{
	...
	isize = sizeof(struct s3_fsal_obj_handle);
	...
	hdl->obj_handle.fileid = atomic_postinc_uint64_t(&s3_inode_number);
//	hdl->datasize = S3.inode_size;
	...
	s3_handle_ops_init(&hdl->obj_handle.obj_ops);

	if (parent != NULL) {
		/* Attach myself to my parent */
		s3_insert_obj(parent, hdl, name);
	} else {
		/* This is an export */
		hdl->is_export = true;
	}
...
#define  s3_int_lookup(d, p, e) _s3_int_lookup(d, p, e, __func__, __LINE__)
static fsal_status_t _s3_int_lookup(struct s3_fsal_obj_handle *dir,
				     const char *path,
				     struct s3_fsal_obj_handle **entry,
				     const char *func, int line)
{
	struct s3_dirent *dirent;
	...
	*entry = NULL;
	...
	dirent = s3_dirent_lookup(dir, path);
	if (!dirent) {
		return fsalstat(ERR_FSAL_NOENT, 0);
	}
	*entry = dirent->hdl;
...
fsal_status_t s3_create_obj(struct s3_fsal_obj_handle *parent,
				    ...
				    struct attrlist *attrs_out)
{
	struct s3_fsal_export *mfe = container_of(op_ctx->fsal_export,
	...
	*new_obj = NULL;		/* poison it */
	...
	status = s3_int_lookup(parent, name, &hdl);
 ...
 * @return FSAL status
 */
static fsal_status_t s3_lookup(struct fsal_obj_handle *parent,
				const char *path,
				struct fsal_obj_handle **handle,
				struct attrlist *attrs_out)
{
	struct s3_fsal_obj_handle *myself, *hdl = NULL;
	fsal_status_t status;
	...
	status = s3_int_lookup(myself, path, &hdl);
	if (FSAL_IS_ERROR(status)) {
		goto out;
	}
...
static fsal_status_t s3_readdir(struct fsal_obj_handle *dir_hdl,
				     ...
				     bool *eof)
{
	struct s3_fsal_obj_handle *myself;
	struct avltree_node *node;
...
//		seekloc = 2;
	...
	*eof = true;
	...
	for (node = avltree_first(&myself->mh_dir.avl_index);
		...
		dirent = avltree_container_of(node, struct s3_dirent, avl_i);
 ...
 *
 * While FSAL_S3 is a support_ex FSAL, it doesn't actually support
 ...
 * @return FSAL status.
 */
static fsal_status_t s3_mkdir(struct fsal_obj_handle *dir_hdl,
			       const char *name,
			       struct attrlist *attrs_in,
			       struct fsal_obj_handle **new_obj,
			       struct attrlist *attrs_out)
{
	struct s3_fsal_obj_handle *parent =
		container_of(dir_hdl, struct s3_fsal_obj_handle, obj_handle);

	LogDebug(COMPONENT_FSAL, "mkdir %s", name);

	return s3_create_obj(parent, DIRECTORY, name, attrs_in, new_obj,
			      attrs_out);
}
 ...
 * @return FSAL status
 */
static fsal_status_t s3_mknode(struct fsal_obj_handle *dir_hdl,
				const char *name, object_file_type_t nodetype,
				struct attrlist *attrs_in,
				struct fsal_obj_handle **new_obj,
				struct attrlist *attrs_out)
{
	struct s3_fsal_obj_handle *hdl, *parent =
		container_of(dir_hdl, struct s3_fsal_obj_handle, obj_handle);
	fsal_status_t status;

	LogDebug(COMPONENT_FSAL, "mknode %s", name);

	status = s3_create_obj(parent, nodetype, name, attrs_in, new_obj,
				attrs_out);
	if (unlikely(FSAL_IS_ERROR(status)))
		return status;

	hdl = container_of(*new_obj, struct s3_fsal_obj_handle, obj_handle);
 ...
 * @return FSAL status
 */
static fsal_status_t s3_getattrs(struct fsal_obj_handle *obj_hdl,
				  struct attrlist *outattrs)
{
	struct s3_fsal_obj_handle *myself =
		container_of(obj_hdl, struct s3_fsal_obj_handle, obj_handle);
 ...
 * @return FSAL status.
 */
fsal_status_t s3_setattr2(struct fsal_obj_handle *obj_hdl,
			   bool bypass,
 ...
 * @return FSAL status
 */
static fsal_status_t s3_unlink(struct fsal_obj_handle *dir_hdl,
				struct fsal_obj_handle *obj_hdl,
				const char *name)
{
	struct s3_fsal_obj_handle *parent, *myself;
...
{
	struct s3_fsal_obj_handle *myself = container_of(obj_hdl,
	...
	status = s3_close_my_fd(&myself->mh_file.fd);
 ...
 * @return FSAL status
 */
static fsal_status_t s3_rename(struct fsal_obj_handle *obj_hdl,
				struct fsal_obj_handle *olddir_hdl,
				const char *old_name,
				struct fsal_obj_handle *newdir_hdl,
				const char *new_name)
{
	struct s3_fsal_obj_handle *s3_olddir =
		container_of(olddir_hdl, struct s3_fsal_obj_handle,
			     obj_handle);
	struct s3_fsal_obj_handle *s3_newdir =
		container_of(newdir_hdl, struct s3_fsal_obj_handle,
			     obj_handle);
	struct s3_fsal_obj_handle *s3_obj =
		container_of(obj_hdl, struct s3_fsal_obj_handle, obj_handle);
	struct s3_fsal_obj_handle *s3_lookup_dst = NULL;
	fsal_status_t status;

	status = s3_int_lookup(s3_newdir, new_name, &s3_lookup_dst);
	if (!FSAL_IS_ERROR(status)) {
		uint32_t numkids;

		if (s3_obj == s3_lookup_dst) {
			/* Same source and destination */
			return status;
		}

		if ((obj_hdl->type == DIRECTORY &&
		     s3_lookup_dst->obj_handle.type != DIRECTORY) ||
		...
		numkids = atomic_fetch_uint32_t(
				&s3_lookup_dst->mh_dir.numkids);
		if (s3_lookup_dst->obj_handle.type == DIRECTORY &&
		    numkids > 2) {
		...
		/* Unlink destination */
		status = s3_unlink(newdir_hdl, &s3_lookup_dst->obj_handle,
				    new_name);
		if (FSAL_IS_ERROR(status)) {
			return status;
		}
	...
	if (!strcmp(old_name, s3_obj->m_name)) {
		/* Change base name */
		gsh_free(s3_obj->m_name);
		s3_obj->m_name = gsh_strdup(new_name);
	}

	/* Insert into new directory */
	s3_insert_obj(s3_newdir, s3_obj, new_name);
 ...
 * @return FSAL status.
 */
fsal_status_t s3_open2(struct fsal_obj_handle *obj_hdl,
			struct state_t *state,
			fsal_openflags_t openflags,
			...
			bool *caller_perm_check)
{
	...
	myself = container_of(obj_hdl, struct s3_fsal_obj_handle, obj_handle);
	...
	status = s3_int_lookup(myself, name, &hdl);
	if (FSAL_IS_ERROR(status)) {
		struct fsal_obj_handle *new_obj;
		...
		status = s3_create_obj(myself, REGULAR_FILE, name, attrs_set,
					&new_obj, attrs_out);
		if (FSAL_IS_ERROR(status)) {
			return status;
		}
		hdl = container_of(new_obj, struct s3_fsal_obj_handle,
				   obj_handle);
		created = true;
	}
	...
	*new_obj = &hdl->obj_handle;
	...
	if (!created) {
		...
		if (setattrs && attrs_set->valid_mask != 0) {
			s3_copy_attrs_mask(attrs_set, &hdl->attrs);
		}
 ...
 * @return FSAL status.
 */
fsal_status_t s3_reopen2(struct fsal_obj_handle *obj_hdl,
			struct state_t *state,
			fsal_openflags_t openflags)
{
	struct s3_fsal_obj_handle *myself =
...
fsal_status_t s3_read2(struct fsal_obj_handle *obj_hdl,
			bool bypass,
			struct state_t *state,
			uint64_t offset,
			size_t buffer_size,
			...
			struct io_info *info)
{
	...
	status = fsal_find_fd(&fsal_fd, obj_hdl, &myself->mh_file.fd,
			      ...
			      FSAL_O_READ, s3_open_func, s3_close_func,
...
fsal_status_t s3_write2(struct fsal_obj_handle *obj_hdl,
			 bool bypass,
			 struct state_t *state,
			 uint64_t offset,
			 size_t buffer_size,
			 ...
			 struct io_info *info)
{
	...
	status = fsal_find_fd(&fsal_fd, obj_hdl, &myself->mh_file.fd,
			      &myself->mh_file.share, bypass, state,
			      FSAL_O_WRITE, s3_open_func, s3_close_func,
			      &has_lock, &closefd, false,
			      &reusing_open_state_fd);
	if (FSAL_IS_ERROR(status)) {
		return status;
...
fsal_status_t s3_commit2(struct fsal_obj_handle *obj_hdl,
			  off_t offset,
			  size_t len)
{
	return fsalstat(ERR_FSAL_NO_ERROR, 0);
 ...
 * @return FSAL status.
 */
fsal_status_t s3_lock_op2(struct fsal_obj_handle *obj_hdl,
			   struct state_t *state,
			   void *owner,
			   ...
			   fsal_lock_param_t *conflicting_lock)
{
	...
	status = fsal_find_fd(&fsal_fd, obj_hdl, &myself->mh_file.fd,
			      &myself->mh_file.share, bypass, state,
			      openflags, s3_open_func, s3_close_func,
...
fsal_status_t s3_close2(struct fsal_obj_handle *obj_hdl,
			 struct state_t *state)
{
	struct fsal_fd *my_fd = (struct fsal_fd *)(state + 1);
	struct s3_fsal_obj_handle *myself = container_of(obj_hdl,
...
static fsal_status_t s3_handle_to_wire(const struct fsal_obj_handle *obj_hdl,
				    uint32_t output_type,
				    struct gsh_buffdesc *fh_desc)
{
	const struct s3_fsal_obj_handle *myself;

	myself = container_of(obj_hdl,
			      const struct s3_fsal_obj_handle,
			      obj_handle);
...
static void s3_handle_to_key(struct fsal_obj_handle *obj_hdl,
			  struct gsh_buffdesc *fh_desc)
{
...
{
	ops->release = s3_release;
	ops->lookup = s3_lookup;
//	ops->merge = s3_merge;
	ops->readdir = s3_readdir;
	ops->mkdir = s3_mkdir;
	ops->mknode = s3_mknode;
	ops->getattrs = s3_getattrs;
	ops->setattr2 = s3_setattr2;
	ops->rename = s3_rename;
	ops->unlink = s3_unlink;
	ops->close = s3_close;
	ops->open2 = s3_open2;
//	ops->status2 = s3_fsal_status2;
	ops->reopen2 = s3_reopen2;
	ops->read2 = s3_read2;
	ops->write2 = s3_write2;
	ops->commit2 = s3_commit2;
	ops->lock_op2 = s3_lock_op2;
	ops->close2 = s3_close2;
	ops->handle_to_wire = s3_handle_to_wire;
...
fsal_status_t s3_lookup_path(struct fsal_export *export_pub,
			     const char *path,
			     struct fsal_obj_handle **pub_handle,
			     struct attrlist *attrs_out)
{
	struct s3_fsal_export *export;
	struct attrlist attrs;
	fsal_status_t status = { ERR_FSAL_NO_ERROR, 0 };

	export = container_of(export_pub, struct s3_fsal_export, export);
	...
	*pub_handle = &export->root_handle->obj_handle;

	if (attrs_out != NULL)
		fsal_copy_attrs(attrs_out, &export->root_handle->attrs, false);
				...
				struct attrlist *attrs_out)
{
	...
	struct s3_fsal_obj_handle *my_hdl;
